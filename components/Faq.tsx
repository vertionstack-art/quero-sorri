      ...
      <Secao id="perguntas">
        <div className="grid gap-10 lg:grid-cols-[22rem_1fr] lg:gap-16">
          ...
          <div className="border-t border-sage-deep">
            {faq.itens.map((item) => (
              <details key={item.p} className="group border-b border-sage-deep">
                <summary className="toque flex items-center justify-between gap-5 py-5 text-left">
                  <h3 className="display text-[1.125rem] leading-snug text-pine">
                    {item.p}
                  </h3>
                  <IconeMais className="h-[1.15rem] w-[1.15rem] shrink-0 text-moss transition-transform duration-200 group-open:rotate-45" />
                </summary>
